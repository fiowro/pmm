...
If the query is described in YAML file with non-default name or location, it
can be added to the |pmm-server| |mysql| metrics with the ``pmm-admin add``
command after the standalone double dash::
